import os
import pathlib

import requests


def get_spacex_links(url):
    response = requests.get(url)
    response.raise_for_status()
    spacex_links = response.json()['links']['flickr']['original']
    return spacex_links


def download_spacex_images(spacex_links, image_folder):
    for link_number, link in enumerate(spacex_links, 1):
        combined_filepath = os.path.join(
            image_folder,
            f'{filename}{link_number}.jpg'
        )
        download_image(combined_filepath, link)


def download_image(combined_filepath, url):
    response = requests.get(url)
    response.raise_for_status()
    with open(combined_filepath, 'wb') as file:
        file.write(response.content)


if __name__ == '__main__':
    filename = 'spacex'
    image_folder = 'images'
    pathlib.Path(image_folder).mkdir(parents=True, exist_ok=True)
    url = 'https://api.spacexdata.com/v4/launches/latest'
    spacex_links = get_spacex_links(url)
    download_spacex_images(image_folder, spacex_links)
